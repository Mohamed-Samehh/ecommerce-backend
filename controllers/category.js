...
async function remove(req, res, next) { // Delete /categories/:id (admin)
  ...
  try {
    ...
    // return res.status(400).send({status: 'fail', message: `Cannot delete ${books.length} books only have ${category.name} category, reassign them first`});
...
async function update(req, res, next) { // Pathc /categories/:id (admin)
  ...
  try {
    ...
    //  return res.status(404).send({status: 'fail', message: 'Category not found'}); // if the id is valid(real objectid) but not in db
